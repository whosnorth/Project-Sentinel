...
## 4. Advanced RAG: Temporal Resolution & Agentic Tool Calling

Users can chat with a specialized AI Analyst that has direct access to the live event database. Basic vector search can struggle with high-precision geopolitical queries or real-time intents, so Sentinel's chat engine relies on a three-pillar architecture:

1. **Smart Temporal Resolution:** When a user asks for "today's events" or "the last 6 hours", semantic search often fails because it matches on vague meaning rather than precise timestamps. Sentinel detects temporal intent at the query parser level and automatically injects a **forced timestamp fetch** (e.g., retrieving the top 25 most severe events strictly from the last 24 hours). This bypasses semantic noise entirely, returning guaranteed real, current data at a fraction of the compute cost.
2. **Hybrid Semantic Search (RRF):** For non-temporal queries (e.g. "What is the impact of the Red Sea blockade?"), Sentinel fuses semantic search (`pgvector` HNSW) with keyword search (GIN Full-Text) using Reciprocal Rank Fusion at the PostgreSQL RPC layer to pull the most relevant historical context.
3. **Agentic Web Search (Tool Calling):** Even with real-time database polling, LLMs can lack immediate breaking facts. The AI Analyst (`deepseek-v4-pro`) is equipped with an agentic tool-calling loop. If a critical fact is missing from the RAG context (e.g., a live commodity price or a breaking news event from the last hour), the model can autonomously invoke the `search_web` tool to query live search APIs (Tavily/Serper). It then loops back to synthesize the live web data with the database context, citing both sources in its final brief.

### Enterprise Security & The BYOD Audit Guard
Agentic tool calling introduces a data leakage risk: what if the LLM accidentally includes proprietary BYOD (Bring Your Own Data) entity names in its external web search query? 
Sentinel solves this natively. Web search is strictly opt-in on a per-organization basis. For opted-in organizations, every single tool call generated by the LLM is deterministically logged to an immutable `sentinel_tool_audit` table. This provides enterprise administrators with 100% transparency into exactly what queries were sent to external providers on their behalf, ensuring strict data governance.
...
1. **Institutional Baseline Aggregation (The Stability Anchor):** Rather than relying on a single source, Sentinel creates a composite baseline stability score, the **Country Stability Index (CSI)**, by merging specific quantitative indices from multiple global institutions.
   ...
   **The Reasoning for Custom Scoring Formulas:** Risk is not an objective, universal constant; it is fundamentally subjective to an institution's operations. A global maritime shipping company evaluating port closures looks at risk entirely differently than a defense contractor evaluating cybersecurity threats. Because every organization has its own specialized parameters for what constitutes a "threat," forcing them into a single universal risk formula dilutes the intelligence. Sentinel supports **Per-Organization Scoring Formulas**. While the platform defaults to a strict baseline weight (`wFSI = 0.25 | wWGI = 0.25 | wACLED = 0.20 | wICRG = 0.20 | wGPI = 0.10`), enterprise administrators can rewrite the math engine, defining their own custom weights and temporal decay rates via the `org_scoring_formulas` table to match their highly specialized threat models.
...
**The Design Philosophy: Absolute Data Sovereignty**
A major friction point in enterprise AI adoption is trust. Hedge funds, defense contractors, and global logistics giants operate on proprietary intelligence (e.g., private supplier vulnerabilities, dark-ship movements, internal risk memos). They fundamentally refuse to upload this highly sensitive data to public SaaS platforms or generalized LLMs because they cannot risk their proprietary edge being absorbed into a model training run or inadvertently leaked into a competitor's chat query. 

Sentinel was engineered from the ground up around the principle of **Absolute Data Sovereignty**. We believe that a true enterprise intelligence platform must bring the analytical engine to the client's data, rather than forcing the client to surrender their data to the engine. Every feature—from BYOD ingestion to BYO-LLM configurations—is built to ensure that an institution's data remains their isolated, strategic advantage.

Sentinel enforces this through a secure, multi-tenant architecture built for strict compliance:
...
*   **The Reasoning for Bring Your Own LLM (BYO-LLM):** Many enterprise clients have already spent millions of dollars training and fine-tuning their own highly specialized AI models specifically for their industry workflows. It is unreasonable to force them to throw away those million-dollar proprietary models just to use our platform. Instead of competing with their internal AI teams, Sentinel acts as the high-velocity orchestration engine *for* them. Sentinel supports a per-organization LLM configuration (`org_llm_configs`), allowing tenants to seamlessly swap the default AI models with their own custom endpoint URLs, model names, and API keys. This guarantees their data remains within their self-hosted infrastructure while allowing them to fully leverage their specialized AI investments.
