...
serve(async (req) => {
  ...
  try {
    ...
    // 1. TRIAGE ROUTER
    const triageModels = ["openai/gpt-4o-mini", "accounts/fireworks/models/deepseek-v4-pro"];
...
Respond STRICTLY in JSON format matching this schema:
{
  ...
  "graph": {
    ...
    if (!triageResult.needs_heavy_extraction && triageResult.graph && triageResult.graph.nodes) {
      ...
      const heavyModels = ["perplexity/llama-3.1-sonar-large-128k-online", "perplexity/llama-3.1-sonar-small-128k-online", "openai/gpt-4o"];
